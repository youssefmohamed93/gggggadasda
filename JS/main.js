...
// Popular Div Tabs // 
let tabs = document.querySelectorAll(".popular-navbar ul li");
let tabsArray = Array.from(tabs);

let contentDivs = document.querySelectorAll(".popular-grid.content > div");
let contentArray = Array.from(contentDivs);


tabsArray[0].addEventListener("click", function () {
    tabsArray.forEach((element) => {
        element.classList.remove("popular-active");
    });
    tabsArray[0].classList.add("popular-active");
...
for (let i = 1; i < tabsArray.length; i++) {
    tabsArray[i].addEventListener("click", function (event) {
        tabsArray.forEach((element) => {
            element.classList.remove("popular-active");
        });
        event.currentTarget.classList.add("popular-active");
        contentDivs.forEach((div) => {
